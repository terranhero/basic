	...
	public sealed class ColumnTypeConverter : TypeConverter
	{
		...
		public override PropertyDescriptorCollection GetProperties(ITypeDescriptorContext context, object value, Attribute[] attributes)
		{
			...
			if (propertyDescriptors == null)
				propertyDescriptors = TypeDescriptor.GetProperties(value, attributes).Sort(new string[] { "Name", "Comment", "DbType",
					"PrimaryKey",  "Size", "Precision",  "Scale", "Nullable", "DefaultValue" });
			DesignColumnInfo column = value as DesignColumnInfo;
			foreach (PropertyDescriptor property in propertyDescriptors)
			{
				if (column.DbType == DbTypeEnum.Binary || column.DbType == DbTypeEnum.VarChar ||
				  column.DbType == DbTypeEnum.Char || column.DbType == DbTypeEnum.NChar ||
				  column.DbType == DbTypeEnum.NVarChar || column.DbType == DbTypeEnum.VarBinary)
				{
					if (property.Name == "Precision" || property.Name == "Scale")
						continue;
				}
				else if (column.DbType == DbTypeEnum.Decimal)
				{
