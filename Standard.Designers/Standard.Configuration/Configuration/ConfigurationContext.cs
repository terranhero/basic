...
namespace Basic.Configuration
{
	...
	public static class ConfigurationContext
	{
		...
		{
			ConfigurationGroup group = GetConfigurationGroup(ConfigurationFilePath);
			SC.ConfigurationSection section = group.Sections[sectionName];
			if (section == null && required) /*读取配置文件异常，配置文件"{0}"中，不存在自定义配置组"'*/
				throw new ConfigurationFileException("Access_Configuration_GroupNotFound", ConfigurationFilePath, sectionName);
			else if (section == null && !required) { return null; }
			T configurationSection = section as T;
			if (configurationSection == null)  /*读取配置文件异常，配置文件"{0}"中，自定义配置组""，类型必须是""*/
