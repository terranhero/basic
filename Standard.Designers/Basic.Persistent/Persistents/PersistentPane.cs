...
namespace Basic.Configuration
{
		...
		IVsFileChangeEvents, IVsDocDataFileChangeControl, IWindowUICommands
	{
		...
		internal EnvDTE.ProjectItem GetEntityProjectItem()
		{
			...
			string[] names = _ProjectItem.Name.Split('.');
			string newEntityName = _ProjectItem.Name.Replace("dpdl", provider.FileExtension);
			if (names != null && names.Length > 0)
			{
				newEntityName = _ProjectItem.Name.Replace(names[names.Length - 1], provider.FileExtension);
			}
			...
			if (_Persistent.Project.NotEmpty)
			{
				...
				if (hierarchy != null && hierarchy.GetProperty(itemId, (int)__VSHPROPID.VSHPROPID_ExtObject, out outProject) >= 0)
				{
					...
					ProjectItem itemModel = dteClass.Solution.FindProjectItem(entityFullName);
					if (itemModel != null) { return itemModel; }
					entityFullName = string.Concat(projectInfo.DirectoryName, "\\", folderName, "\\", newEntityName);
			...
			foreach (EnvDTE.ProjectItem item in _ProjectItem.ProjectItems)
			{
				if (item.ProjectItems.Count > 0)
				{
					...
					{
						if (childItem.Name == entityName || childItem.Name == newEntityName) { return childItem; }

					}
				}
				if (item.Name == entityName || item.Name == newEntityName) { return item; }
			}
			return null;
		}
