	...
	public sealed partial class PersistentService
	{
		...
		public STT.Task InitializeMenuAsync(CancellationToken cancellationToken, IProgress<ServiceProgressData> progress)
		{
			...
			if (null != _oleMenuService)
			{
				...
				_oleMenuService.AddCommand(IDC_REGISTER_FILE_EXTENSION, OnRegisterFileExtension, OnCanRegisterFileExtension);
		...
		private void OnCreateResource(object sender, EventArgs e)
		{
			try
			{
				...
				AddResourceDialog resourceView = new AddResourceDialog(_Persistent, resourceCollection) { Width = 900, Height = 430 };
				if (resourceView.ShowModal() == true)
				{
					SaveResourcesToFile(_Persistent, dteClass, resourceCollection);
		...
		private void SaveResourcesToFile(PersistentDesigner persistent, EnvDTE80.DTE2 dteClass, LocalizationCollection resources)
		{
			System.IO.FileInfo fileSolution = new System.IO.FileInfo(dteClass.Solution.FullName);
			string localFilePath = string.Concat(fileSolution.DirectoryName, persistent.MessageConverter.FileName);
			EnvDTE.ProjectItem _ProjectItem = dteClass.Solution.FindProjectItem(localFilePath);

			this.SetWaitCursor();
			if (_ProjectItem == null)
			{
				string message = string.Concat("本地化资源文件\"", persistent.MessageConverter.ConverterName, "\"不存在，请重新选择。");
			...
			if (pane == null)
			{
				string message = string.Concat("本地化资源文件\"", persistent.MessageConverter.ConverterName,
					"\"打开失败，可能原因是本地化资源编辑器已经更改。");
				this.ShowMessage(message); return;
			}
			ResourceEditor editor = pane.Content as ResourceEditor;
			foreach (LocalizationItem resx in resources)
			{
		...
		private void OnCreateCommandResource(object sender, EventArgs e)
		{
			try
			{
				...
				AddResourceDialog resourceView = new AddResourceDialog(_Persistent, notExistsCollection) { Width = 900, Height = 430 };
				if (resourceView.ShowModal() == true)
				{
					SaveResourcesToFile(_Persistent, dteClass, notExistsCollection);
					string resInfo = StringUtils.GetString("Package_ResourceDescription");
					string successInfo = StringUtils.GetString("Package_CreateResourceSuccessful"); //当前数据持久类资源已经创建成功，且已经复制进系统剪贴板。
					ShowMessage(successInfo, resInfo);
				}
