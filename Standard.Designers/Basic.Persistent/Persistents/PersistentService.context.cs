...
namespace Basic.Configuration
{
	...
	public sealed partial class PersistentService
	{
		...
		/// <summary>注册文件扩展名</summary>
		public static readonly CommandID IDC_REGISTER_FILE_EXTENSION = new CommandID(PersistentGuid, 0x0520);
			...
			foreach (EnvDTE.UIHierarchyItem uitem in array)
			{
				if (uitem.Object is EnvDTE.ProjectItem)
				{
					...
					if (item.Kind == EnvDTE.Constants.vsProjectItemKindPhysicalFolder)
					{
						...
						if (ReadJson(item))
						{
							ShowCreatePersistent(item.ProjectItems, fullPath, defaultNamespace);
						}
					}
				}
				else if (uitem.Object is EnvDTE.Project)
				{
					...
					if (ReadJson(project))
					{
						ShowCreatePersistent(project.ProjectItems, fullPath, defaultNamespace);
					}
				}
			}
		}

		private void ShowCreatePersistent(EnvDTE.ProjectItems items, string fullPath, string defaultNamespace)
		{
			...
			_IVsUIShell.GetDialogOwnerHwnd(out IntPtr hwnd);
			...
			if (WindowHelper.ShowModal(form, hwnd) == 1)
			{
				...
				EnvDTE.ProjectItem item = items.AddFromFile(filePath);
		...
		private void AddLocalizationFile(EnvDTE.ProjectItems items)
		{
			...
			LocalizationDialog dialog = new LocalizationDialog(fullPath, "Strings")
			{
				ShowActivated = true,
				ShowInTaskbar = true,
				Width = 700,
