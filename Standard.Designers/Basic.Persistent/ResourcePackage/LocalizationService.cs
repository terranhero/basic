	...
	public sealed partial class LocalizationService
	{
		...
		internal async STT.Task InitializeAsync(CancellationToken cancellationToken, IProgress<ServiceProgressData> progress)
		{
			...
			if (null != oleMenuService)
			{
				...
				oleMenuService.AddCommand(IDC_SVN_VERSION, OnVersion, OnCanVersion);
		...
		private void OnCanVersion(object sender, EventArgs e)
		{
			ThreadHelper.ThrowIfNotOnUIThread();
			...
			//{
			//	EnvDTE.TextSelection textSelection = (EnvDTE.TextSelection)dte2Class.ActiveDocument.Selection;
			//	if (textSelection == null) { return; }
			//	try
			//	{
			//		if (string.IsNullOrWhiteSpace(textSelection.Text) == false)
			//		{
			//			string text = textSelection.Text;
			//			if (reg1.IsMatch(text)) { menu.Visible = menu.Enabled = true; }
			//			else if (reg2.IsMatch(text)) { menu.Visible = menu.Enabled = true; }
			//			else if (reg3.IsMatch(text)) { menu.Visible = menu.Enabled = true; }
			//			else if (reg4.IsMatch(text)) { menu.Visible = menu.Enabled = true; }
			//		}
			//	}
			//	catch { menu.Visible = menu.Enabled = false; }
			//}
		}

		private readonly Regex reg4 = new Regex(@"[assembly: AssemblyFileVersion\(""[0-9]+.[0-9]+.[0-9]+[.0-9]*""\)]", RegexOptions.Singleline);
		private readonly Regex reg3 = new Regex(@"[assembly: AssemblyFileVersionAttribute\(""[0-9]+.[0-9]+.[0-9]+[.0-9]*""\)]", RegexOptions.Singleline);
		private readonly Regex reg2 = new Regex(@"[assembly: System.Reflection.AssemblyFileVersion\(""[0-9]+.[0-9]+.[0-9]+[.0-9]*""\)]", RegexOptions.Singleline);
		private readonly Regex reg1 = new Regex(@"[assembly: System.Reflection.AssemblyFileVersionAttribute\(""[0-9]+.[0-9]+.[0-9]+[.0-9]*""\)]", RegexOptions.Singleline);

		private readonly Regex reg = new Regex(@"^""[0-9]+.[0-9]+.[0-9]+[.0-9]*""$", RegexOptions.Singleline);
		/// <summary>提取最新提交日志版本号</summary>
		private void OnVersion(object sender, EventArgs e)
		{
			...
			{
				Regex reg = new Regex(@"(?<=<BuildVersion>)(.*?)(?=</BuildVersion>)", RegexOptions.Singleline);
				string result;
				using (StreamReader reader = new StreamReader(fileInfo.FullName))
				{
					result = reg.Replace(reader.ReadToEnd(), Convert.ToString(revision));
				}
				if (string.IsNullOrWhiteSpace(result) == true) { return; }
			...
			{
				//[assembly: System.Reflection.AssemblyFileVersion("6.0.5392")], [0-9]+.[0-9]+.[0-9]+[.0-9]*

				EnvDTE.TextSelection textSelection = (EnvDTE.TextSelection)dte2Class.ActiveDocument.Selection;
				if (textSelection == null) { return; }
				if (string.IsNullOrWhiteSpace(textSelection.Text) == false)
				{
					string text = textSelection.Text; string result;
					using (StreamReader reader = new StreamReader(fileInfo.FullName))
					{
						Match match1 = reg1.Match(text);
						if (match1.Success) { string value1 = match1.Value; }
						else if (reg2.IsMatch(text)) { string value1 = match1.Value; }
						else if (reg3.IsMatch(text)) { string value1 = match1.Value; }
						else if (reg4.IsMatch(text)) { string value1 = match1.Value; }

						result = reg1.Replace(text, Convert.ToString(revision));
					}
					if (string.IsNullOrWhiteSpace(result) == true) { return; }
		...
		private void OnCanExtractEnum(object sender, EventArgs e)
		{
			...
			if (dteClass == null) { menu.Visible = menu.Enabled = false; return; }
			if (dte.ActiveDocument.Language != "CSharp") { menu.Visible = menu.Enabled = false; return; }
			EnvDTE.TextSelection textSelection = (EnvDTE.TextSelection)dte.ActiveDocument.Selection;
			if (textSelection == null) { menu.Visible = menu.Enabled = false; return; }
			EnvDTE.VirtualPoint point = textSelection.ActivePoint;
			if (point == null) { menu.Visible = menu.Enabled = false; return; }
			EnvDTE.CodeElement element = point.get_CodeElement(EnvDTE.vsCMElement.vsCMElementEnum);
			if (element == null) { menu.Visible = menu.Enabled = false; return; }
		...
		private void OnExtractEnum(object sender, EventArgs e)
		{
			...
			if (resourceView.ShowModal() == true)
			{
				...
				MessageInfo info = resourceView.MessageConverter; string fullName = info.FileName;
				EnvDTE.ProjectItem item = dte.Solution.FindProjectItem(fullName);
				if (item == null)
				{
					string message = string.Concat("本地化资源文件\"", info.ConverterName, "\"已经不存在，请重新选择。");
					this.ShowMessage(message); return;
				}
				if (item.IsOpen == true) { item.Document.Activate(); }
				else { EnvDTE.Window window = item.Open(); window.Visible = true; }

				if (item.Document.ActiveWindow.Object is LocalizationPane pane)
				{
					ResourceEditor editor = pane.Content as ResourceEditor;
					foreach (LocalizationItem resx in resourceCollection)
					{
