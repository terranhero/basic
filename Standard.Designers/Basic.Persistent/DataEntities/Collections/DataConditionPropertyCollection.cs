...
namespace Basic.Collections
{
	...
	public sealed class DataConditionPropertyCollection : Basic.Collections.AbstractCollection<DesignerDataConditionProperty>,
		ICollection<DesignerDataConditionProperty>, IEnumerable<DesignerDataConditionProperty>, INotifyCollectionChanged
	{
		/// <summary>
		/// 数据库表中所有列配置节名称
		/// </summary>
		internal const string XmlElementName = "ConditionProperties";
		internal readonly DesignerDataCondition _DataCondition;
		/// <summary>
		/// 初始化 DataEntityPropertyCollection 类的新实例。
		/// </summary>
		internal DataConditionPropertyCollection(DesignerDataCondition dataCondition)
		...
		protected internal override string GetKey(DesignerDataConditionProperty item)
		{
		...
		protected override void InsertItem(int index, DesignerDataConditionProperty item)
		{
		...
		{
			if (name == DesignerDataCondition.BaseClassAttribute) { _DataCondition.BaseClass = value; return true; }
			else if (name == AbstractDesignerEntity.ExpandedAttribute)
			{
				if (bool.TryParse(value, out bool _Expanded))
				{ _DataCondition.Expanded = _Expanded; return true; }
				else { return false; }
			}
			else if (name == AbstractDesignerEntity.GuidAttribute)
			{
		...
		{
			DesignerDataConditionProperty element = new DesignerDataConditionProperty(_DataCondition);
