	...
	internal class DynamicCommandElement : DataCommandElement, IXmlSerializable
	{
		...
		public override string MethodName
		{
			get
			{
				...
				else if (Kind == ConfigurationTypeEnum.SearchTable && ExecutableMethod == DynamicMethodEnum.GetJoinEntities && AsyncGenerated == false)
					return "GetJoinEntities";
				else if (Kind == ConfigurationTypeEnum.SearchTable && ExecutableMethod == DynamicMethodEnum.GetJoinEntities && AsyncGenerated == true)
					return "GetJoinEntitiesAsync";
		...
		protected internal override void WriteDesignerCode(CodeTypeDeclaration classCode, int targetFramework)
		{
			...
			if (_ExecutableMethod == DynamicMethodEnum.GetEntities)
				GenerateDesignerGetEntities(classCode.Members, false);
			else if (_ExecutableMethod == DynamicMethodEnum.GetJoinEntities)
				GenerateDesignerGetEntities(classCode.Members, true);
		...
		/// <summary>表示执行 GetJoinEntities 方法的命令。</summary>
		/// <param name="members">当前类型申明的 CodeTypeMemberCollection 。</param>
		/// <param name="method">需要定义的当前方法</param>
		...
		protected override void WriteContextDesignerCode(CodeTypeMemberCollection members, CodeMemberMethod method)
		{
			...
			else if (ExecutableMethod == DynamicMethodEnum.GetJoinEntities)
				GenerateContextGetJoinEntities(members, method);
