...
namespace Basic.Localizations
{
	...
	public sealed class ResourceGenerator : IVsSingleFileGenerator, IObjectWithSite, IDisposable
	{
		...
		private void WriteEntityDesignerCode(CodeCompileUnit codeComplieUnit, string wszInputFilePath)
		{
			...
			EnvDTE.Property rootNamespace = projectItem.ContainingProject.Properties.Item("RootNamespace");
			string resNamespace = Convert.ToString(rootNamespace.Value);
			EnvDTE.Property property = projectItem.Properties.Item("DefaultNamespace");
			EnvDTE.ProjectItem folderItem = projectItem.Collection.Parent as EnvDTE.ProjectItem;
			if (folderItem != null)
			{
				property = folderItem.Properties.Item("DefaultNamespace");
				resNamespace = Convert.ToString(property.Value);
			}

			string outputNamespace = resNamespace;
