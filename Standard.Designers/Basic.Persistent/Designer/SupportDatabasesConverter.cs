...
namespace Basic.Designer
{
	...
	public sealed class SupportDatabasesConverter : TypeConverter
	{
		...
		{
			if (value is string types)
			{
				ConnectionTypes result = ConnectionTypes.Default;
				List<ConnectionTypes> list = new List<ConnectionTypes>(10);
				foreach (string str in types.Split(','))
				{
					if (str == "NPGSQL") { list.Add(ConnectionTypes.PGSQL); }
					else if (Enum.TryParse<ConnectionTypes>(str, out result))
					{
						list.Add(result);
					}
				}
		...
		public override object ConvertTo(ITypeDescriptorContext context, System.Globalization.CultureInfo culture, object value, Type destinationType)
		{
			if (value is ConnectionTypes[])
			{
				...
				foreach (ConnectionTypes element in (ConnectionTypes[])value)
				{
					list.Add(Enum.GetName(typeof(ConnectionTypes), element));
				}
