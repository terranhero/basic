...
namespace Basic.Localizations
{
	...
	public sealed class LocalizationCollection : Collection<LocalizationItem>, IXmlSerializable, INotifyCollectionChanged
	{
		...
		internal const string ResxDatasElementName = "resxDatas";
		...
		{
			CollectionChanged?.Invoke(this, new NotifyCollectionChangedEventArgs(action)); OnContentChanged(EventArgs.Empty);
		}
		...
		{
			CollectionChanged?.Invoke(this, e); OnContentChanged(EventArgs.Empty);
		}
		#endregion
			...
			: base()
		{
			...
			{
				new CultureInfo(0x0004),    //zh-Hans
				new CultureInfo(0x7C04),    //zh-Hant
				new CultureInfo(2052),  //zh-CN
				new CultureInfo(1028),  //zh-TW
				new CultureInfo(1033)   //en-US
			};
			//enabledCultureInfos.Add(
		}
		...
		/// </summary>
		/// <param name="cultureInfo"></param>
		internal void AddEnabledCultureInfos()
		{
		...
		/// 加载本地化字符串资源信息。
		/// </summary>
		/// <param name="item">解决方案中本地化资源文件节点。</param>
		...
		/// <param name="pszFilename">本地化资源文件全名，含路径。</param>
		/// <returns>如果加载成功，则返回True，否则返回False</returns>
		public bool LoadResource(EnvDTE.ProjectItem item, string pszFilename)
		{
			ThreadHelper.ThrowIfNotOnUIThread();
			if (item == null || !File.Exists(pszFilename)) { return false; }
			using (XmlReader reader = XmlReader.Create(pszFilename))
			{
				this.ReadXml(reader);
			}
			EnvDTE.DTE dte = item.DTE;
			FileInfo fileInfo = new FileInfo(pszFilename); 
			_FileName = Path.GetFileNameWithoutExtension(fileInfo.FullName);//.Replace(fileInfo.Extension, "");
			foreach (CultureInfo cultureInfo in cultureInfos)
			{
				string cultureFileName = string.Concat(pszFilename.Replace(fileInfo.Extension, ""), ".", cultureInfo.Name, ".resx");
				EnvDTE.ProjectItem cultureItem = dte.Solution.FindProjectItem(cultureFileName);
				FileInfo cultureFileInfo = new FileInfo(cultureFileName);
				if (!cultureFileInfo.Exists) { continue; }
				using (ResXResourceReader reader = new ResXResourceReader(cultureFileInfo.OpenRead()))
				{
					foreach (DictionaryEntry node in reader)
					{
						...
						if (resourceCollection.TryGetValue(name, out LocalizationItem localResx))
						{
							if (localResx.ContainsKey(cultureInfo.Name))
								localResx[cultureInfo.Name] = node.Value as string;
							else
								localResx.Add(cultureInfo.Name, node.Value as string);
						}
		...
		/// <param name="pszFilename">本地化资源文件全名，含路径。</param>
		/// <returns>如果加载成功，则返回True，否则返回False</returns>
		public bool SaveResource(EnvDTE.ProjectItem item, string pszFilename)
		{
			ThreadHelper.ThrowIfNotOnUIThread();
			...
			EnvDTE.ProjectItem resxProjectItem = dte.Solution.FindProjectItem(resxName);
			...
			if (resxProjectItem == null) { _ = item.ProjectItems.AddFromFile(resxName); }
			foreach (CultureInfo cultureInfo in cultureInfos)
			{
				string cultureFileName = itemInfo.FullName.Replace(itemInfo.Extension, string.Concat(".", cultureInfo.Name, ".resx"));
				if (!File.Exists(cultureFileName)) { continue; }
				using (ResXResourceWriter writer = new ResXResourceWriter(cultureFileName))
				{
					foreach (LocalizationItem localResx in this)
					{
						ResXDataNode node = new ResXDataNode(localResx.Name, localResx[cultureInfo.Name])
						{
							Comment = localResx.Comment
						};
						writer.AddResource(node);
		...
		internal void WriteXml(System.Xml.XmlWriter writer)
		{
			...
			foreach (LocalizationItem localizationResource in sortedList)
			{
				localizationResource.WriteXml(writer);
			}
			writer.WriteEndElement();
			writer.WriteEndElement();
		}
	...
	/// </summary>
	public sealed class GroupNameCollection : System.Collections.ObjectModel.Collection<string>, INotifyCollectionChanged
	{
		/// <summary>
