...
where t1.object_id=object_id('{0}')", tableInfo.Name);
			...
			using (SqlDataReader reader = sqlCommand.ExecuteReader())
			{
				...
				while (reader.Read())
				{
					...
					info.Name = reader.GetString(nameIndex);
					info.PropertyName = StringHelper.GetCsName(info.Name);
				...
				using (SqlDataReader reader = sqlCommand.ExecuteReader())
				{
					...
					while (reader.Read())
					{
						...
						if (info.TableName != null && info.TableName.IndexOf('_') >= 0)
							info.EntityName = StringHelper.GetCsName(info.TableName.Substring(info.TableName.IndexOf('_') + 1,
								info.TableName.Length - info.TableName.IndexOf('_') - 1));
						else
							info.EntityName = StringHelper.GetCsName(info.TableName);
			...
			using (SqlDataReader reader = sqlCommand.ExecuteReader())
			{
				...
				while (reader.Read())
				{
					...
					if (info.Name != null && info.Name.IndexOf('_') >= 0)
						info.EntityName = StringHelper.GetCsName(info.Name.Substring(info.Name.IndexOf('_') + 1,
							info.Name.Length - info.Name.IndexOf('_') - 1));
					else
						info.EntityName = StringHelper.GetCsName(info.Name);
					tableList.Add(info);
				}
			}
			...
			using (SqlDataReader reader = sqlCommand.ExecuteReader(CommandBehavior.SchemaOnly | CommandBehavior.KeyInfo))
			{
				...
				foreach (DataRow row in table.Rows)
				{
					...
					if (string.IsNullOrWhiteSpace(column.PropertyName))
						column.PropertyName = StringHelper.GetCsName(columnName);
