	...
	public sealed partial class PersistentService
	{
		...
		private void OnUpdateEntities(object sender, EventArgs e)
		{
			try
			{
				...
				DataEntityElement newEntity = null, selectEntity = null;
				DataEntityElement updateEntity = null, deleteEntity = null;
		...
		private void OnPaste(object sender, EventArgs e)
		{
			...
			else if (Clipboard.ContainsText(TextDataFormat.UnicodeText))
			{
				...
				if (canvas.SelectedItem != null)
				{
					...
					{
						DynamicCommandElement dynamicCommand = new DynamicCommandElement(entityEntityElement);
						dynamicCommand.Name = string.Concat("DynamicCommand_", entityEntityElement.DataCommands.Count);
						if (TransactSqlResolver.PasteDynamicCommand(dynamicCommand, text))
						{
							Clipboard.Clear();
							entityEntityElement.DataCommands.Add(dynamicCommand);
