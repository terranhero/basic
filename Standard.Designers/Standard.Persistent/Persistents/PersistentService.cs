...
namespace Basic.Configuration
{
	...
	public sealed partial class PersistentService
	{
		...
		public async STT.Task InitializeAsync(IProgress<ServiceProgressData> progress)
		{
			...
			_oleMenuService = await asyncPackage.GetServiceAsync(typeof(IMenuCommandService)).ContinueWith(task =>
			{
				return task.Result as OleMenuCommandService;
			}, STT.TaskScheduler.Default);
			_IVsUIShell = await asyncPackage.GetServiceAsync(typeof(SVsUIShell)).ContinueWith(task =>
			{
				return task.Result as IVsUIShell;
			}, STT.TaskScheduler.Default);
			_VsSolution = await asyncPackage.GetServiceAsync(typeof(SVsSolution)).ContinueWith(task =>
			{
				return (IVsSolution)task.Result;
			}, STT.TaskScheduler.Default);
			Assumes.Present(_VsSolution);
			_DteClass = await asyncPackage.GetServiceAsync(typeof(SDTE)).ContinueWith(task =>
			{
				return (EnvDTE80.DTE2)task.Result;
			}, STT.TaskScheduler.Default);
		...
		private void OnResetResource(object sender, EventArgs e)
		{
			try
			{
				...
				StringBuilder resBuilder = new StringBuilder(resInfo, 1000);
				int resLength = resBuilder.Length;
				...
				if (resLength != resBuilder.Length) { ShowMessage(resBuilder.ToString()); }
			}
			catch (Exception ex)
			{
				ShowMessage(ex);
