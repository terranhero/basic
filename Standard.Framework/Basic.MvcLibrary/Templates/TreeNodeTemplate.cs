...
namespace Basic.MvcLibrary
{
	...
	public sealed class TreeNodeTemplate<T> : ITemplateView where T : class
	{
		...
		internal TreeNodeTemplate(IBasicContext bh)
		{
			mBasic = bh; writer = new TagHtmlWriter(ViewTags.Template);
			writer.MergeAttribute("v-slot:node", "{node, data}");
		...
		internal TreeNodeTemplate(IBasicContext bh, IOptions opts)
		{
			mBasic = bh; writer = new TagHtmlWriter(ViewTags.Template);
