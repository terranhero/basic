...
namespace Basic.MvcLibrary
{
	...
	public sealed class CheckList : Input<CheckList>
	{
		private readonly IBasicContext basicContext;
		private readonly EntityPropertyMeta propertyMeta;
		/// <summary>初始化 CheckList 类实例</summary>
		/// <param name="basic">表示请求上下文信息</param>
		/// <param name="meta">表示属性的元数据</param>
		internal CheckList(IBasicContext basic, EntityPropertyMeta meta)
			: base(basic, meta, InputTags.CheckboxGroup) { basicContext = basic; propertyMeta = meta; }

		/// <summary>初始化空实例，此不输出。</summary>
		private CheckList(bool empty) : base(empty) { }

		/// <summary>表示空实例</summary>
		public static CheckList Empty { get { return new CheckList(true); } }

		private string Option<T>(string tag, T label, string text)
		{
			TagHtmlWriter writer = new TagHtmlWriter(tag);

			if (label is string)
			{
				writer.MergeAttribute("key", Convert.ToString(label));
				writer.MergeAttribute("label", label as string);
			}
			...
			{
				writer.MergeAttribute(":key", Convert.ToString(label));
				writer.MergeAttribute(":label", Convert.ToString(label));
			}
			else
			{
				writer.MergeAttribute("key", Convert.ToString(label));
			...
			//writer.MergeAttribute(":disabled", disabled ? "true" : "false");
			//writer.MergeAttribute(":checked", selected ? "true" : "false");
		...
		{
			TagHtmlWriter writer = new TagHtmlWriter(tag);
			writer.MergeAttribute("key", label);
		...
		private CheckList Options(string tag)
		{
			...
			else if (propertyMeta.PropertyType.IsEnum)
			{
				...
				foreach (object value in valueArray)
				{
					...
					//TagHtmlWriter writer = new TagHtmlWriter(tag);
					//writer.MergeAttribute("key", Convert.ToString((int)value));
					//writer.MergeAttribute("label", (int)value);
					string itemName = string.Concat(enumName, "_", name);
					string text = basicContext.GetString(converterName, itemName);

					if (string.IsNullOrWhiteSpace(text) == false) { builder.AppendLine(Option(tag, (int)value, text)); }
					else { builder.AppendLine(Option(tag, (int)value, name)); }
			...
			else if (propertyMeta.PropertyType == typeof(bool) || propertyMeta.PropertyType == typeof(bool?))
			{
				...
				tWriter.MergeAttribute("key", "true");
				...
				fWriter.MergeAttribute("key", "false");
		...
		//public CheckList Options<T>(IEnumerable<T> collection) where T : ISelectOption
		//{
		...
		//		{
		//			stringBuilder.AppendLine(Option(InputTags.Checkbox, item2.Value, item2.Text, item2.Attributes));
		//		}
		...
		public CheckList Options() { return Options(InputTags.Checkbox); }

		/// <summary>输出选择项信息</summary>
		/// <returns>返回当前对象。</returns>
		public CheckList Buttons() { return Options(InputTags.CheckboxButton); }

		/// <summary>输出单个单选按钮</summary>
		private CheckList OptionFor<TE>(string tag, TE value)
		{
		...
		/// <summary>输出单个单选按钮</summary>
		public CheckList OptionFor<TE>(TE value)
		{
			return OptionFor<TE>(InputTags.Checkbox, value);
		}

		/// <summary>输出单个单选按钮</summary>
		public CheckList ButtonFor<TE>(TE value)
		{
			return OptionFor<TE>(InputTags.CheckboxButton, value);
		}

		/// <summary>作为 value 唯一标识的键名，绑定值为对象类型时必填 </summary>
		/// <returns>返回当前对象。</returns>
		public CheckList ValueKey(string value) { SetProp("value-key", value); return this; }

		/// <summary>多选模式下是否折叠Tag </summary>
		/// <returns>返回当前对象。</returns>
		public CheckList CollapseTags() { SetProp("collapse-tags", true); return this; }

		/// <summary>选项分隔符 </summary>
		/// <returns>返回当前对象。</returns>
		public CheckList SetMultiple() { SetProp("multiple", true); return this; }

		/// <summary>是否可清空</summary>
		/// <returns>返回当前对象。</returns>
		public CheckList Clearable() { SetProp("clearable", true); return this; }

		/// <summary>输入框中是否显示选中值的完整路径，</summary>
		/// <param name="value">输入框是否显示选中值的完整路径，默认值为 true</param>
		/// <returns>返回当前对象。</returns>
		public CheckList ShowAllLevels(bool value) { SetProp("show-all-levels", value); return this; }

		/// <summary>输入框中是否显示选中值的完整路径，默认值为 true</summary>
		/// <returns>返回当前对象。</returns>
		public CheckList HideAllLevels() { SetProp("show-all-levels", false); return this; }

		/// <summary>是否可搜索选项，</summary>
		/// <param name="value">是否可搜索选项，默认值为 true</param>
		/// <returns>返回当前对象。</returns>
		public CheckList Filterable(bool value) { SetProp("filterable", value); return this; }

		/// <summary>原生属性，自动补全(on, off),默认off</summary>
		/// <returns>返回当前对象。</returns>
		public CheckList AutoComplete() { SetAttr("autocomplete", "on"); return this; }

		/// <summary>输出开始标签</summary>
		/// <returns>返回当前对象</returns>
		public new CheckList Begin() { return base.Begin() as CheckList; }
		...
		/// <returns>返回 CheckList 的 Html 字符串</returns>
		public override string ToString()
		{
			base.AddClass("web-CheckList");
