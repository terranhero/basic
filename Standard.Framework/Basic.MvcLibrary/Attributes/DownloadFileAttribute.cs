...
namespace Basic.MvcLibrary
{
	...
	/// 下载文件自定义名称特性
	/// </summary>
	public sealed class DownloadFileAttribute : FilterAttribute, IActionFilter
	{
		/// <summary>
		/// 下载文件在视图字典的键值。
		/// </summary>
		...
		public void OnActionExecuting(ActionExecutingContext filterContext)
		{
			...
			if (_IsLocaltion)
			{
				fileName = request.GetString(_ConverterName, resourceName);
				filterContext.HttpContext.Request.Headers.Add(ViewDataKey, fileName);
				filterContext.Controller.ViewData[ViewDataKey] = fileName;
				return;
			}
			filterContext.HttpContext.Request.Headers.Add(ViewDataKey, _DownloadFileName);
			filterContext.Controller.ViewData[ViewDataKey] = _DownloadFileName;
		}
	}
}
