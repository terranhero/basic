...
namespace Basic.EasyLibrary
{
	...
	public sealed class DataGridArraysColumn<T, TR> : DataGridArraysColumn<T>
		where T : class
		where TR : struct, IComparable<TR>, IEquatable<TR>
	{
		...
		internal DataGridArraysColumn(IBasicContext httpContext, Func<T, IEnumerable<PropertyMeta<TR>>> valueFunc,
			EntityPropertyMeta metaData, IDictionary<string, IDictionary<string, string>> fields)
			: base(httpContext, fields) { _ValueFunc = valueFunc; _PropertyMeta = metaData; }

		internal Func<IEnumerable<T>, IDictionary<string, TR>> _AggFunc;
		/// <summary>
		/// <![CDATA[计算序列IEnumerable<T>的聚合。]]>
		/// </summary>
		/// <returns>返回当前列对象。</returns>
		public DataGridArraysColumn<T, TR> Grouping(Func<IEnumerable<T>, IDictionary<string, TR>> aggFunc)
		{
		...
		public override IDictionary<string, object> GetValues(object model)
		{
			...
			return result1.ToDictionary(m => m.Key, m =>
			{
				object result = m.Value; if (string.IsNullOrWhiteSpace(Format) == false) { return string.Format(Format, result); }
	...
	/// 表示数组类列
	/// </summary>
	public abstract class DataGridArraysColumn<T> : DataGridColumn<T>
	{
		private readonly IDictionary<string, IDictionary<string, string>> _Fields;
		/// <summary>
		/// 初始化 DataGridArraysColumn 类实例
		/// </summary>
		/// <param name="context">当前 HTTP 上下文信息。</param>
		/// <param name="fields">数组列字段信息</param>
		protected DataGridArraysColumn(IBasicContext context, IDictionary<string, IDictionary<string, string>> fields) : base(context) { _Fields = fields; }

		/// <summary>
		/// 数组列字段列表
		/// </summary>
		public IDictionary<string, IDictionary<string, string>> Fields { get { return _Fields; } }
