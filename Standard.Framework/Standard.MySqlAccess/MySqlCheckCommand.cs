...
namespace Basic.MySqlAccess
{
	...
	public class MySqlCheckCommand : CheckCommand, ICloneable
	{
		private readonly MySqlCommand _MySqlCommand;
		private readonly MySqlStaticCommand dbStaticCommand;
		/// <summary>
		/// 执行SQL Server 类型数据库的数据检测。
		/// </summary>
		/// <param name="staticCommand">表示执行静态命令 StaticCommand 类实例。</param>
		internal MySqlCheckCommand(MySqlStaticCommand staticCommand)
			: this(staticCommand, new MySqlCommand()) { }
		...
		private MySqlCheckCommand(MySqlStaticCommand staticCommand, MySqlCommand sqlCmd)
			: base(staticCommand, sqlCmd) { _MySqlCommand = sqlCmd; dbStaticCommand = staticCommand; }

		/// <summary>当前命令的数据库类型</summary>
		public override ConnectionType ConnectionType { get { return ConnectionType.MySqlConnection; } }

		/// <summary>
		///  获取 System.Data.SqlClient.MySqlParameterCollection。
		/// </summary>
		/// <value>Transact-SQL 语句或存储过程的参数。默认值为空集合。</value>
		[DesignerSerializationVisibility(DesignerSerializationVisibility.Content)]
		public new MySqlParameterCollection Parameters
		{
			get { return _MySqlCommand.Parameters; }
		}
		...
		/// </summary>
		public MySqlCheckCommand Clone() { return Clone(dbStaticCommand); }

		/// <summary>
		/// 复制 CheckDataCommand 命令。
		/// </summary>
		internal MySqlCheckCommand Clone(MySqlStaticCommand staticCommand)
		{
			lock (this)
			{
				MySqlCheckCommand checkCommand = new MySqlCheckCommand(staticCommand, (MySqlCommand)_MySqlCommand.Clone());
		...
		internal protected override void ReadOwnerCommandParameter(string[] strArray)
		{
			foreach (string parameterName in strArray)
			{
				...
				{
					MySqlParameter param = dataCommand.Parameters[paramName] as MySqlParameter;
					MySqlParameter newParameter = (param as ICloneable).Clone() as MySqlParameter;
					_MySqlCommand.Parameters.Add(newParameter);
				}
			}
		}
