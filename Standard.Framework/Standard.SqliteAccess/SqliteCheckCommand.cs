...
namespace Basic.SqliteAccess
{
	...
	public class SqliteCheckCommand : CheckCommand, ICloneable
	{
		private readonly SqliteCommand _sqliteCommand;
		private readonly SqliteStaticCommand dbStaticCommand;
		/// <summary>
		/// 执行SQL Server 类型数据库的数据检测。
		/// </summary>
		/// <param name="staticCommand">表示执行静态命令 StaticCommand 类实例。</param>
		internal SqliteCheckCommand(SqliteStaticCommand staticCommand)
			: base(staticCommand, new SqliteCommand())
		{
			dbStaticCommand = staticCommand;
			_sqliteCommand = dataDbCommand as SqliteCommand;
		}
		...
		private SqliteCheckCommand(SqliteStaticCommand staticCommand, SqliteCommand sqlCmd)
			: base(staticCommand, sqlCmd) { _sqliteCommand = sqlCmd; dbStaticCommand = staticCommand; }
		...
		{
			get { return _sqliteCommand.Parameters; }
		}
		...
		/// </summary>
		public SqliteCheckCommand Clone() { return Clone(dbStaticCommand); }

		/// <summary>
		/// 复制 CheckDataCommand 命令。
		/// </summary>
		/// <param name="staticCommand">复制后 SqlCheckCommand 所属的静态命令。</param>
		internal SqliteCheckCommand Clone(SqliteStaticCommand staticCommand)
		{
			lock (this)
			{
				SqliteCheckCommand checkCommand = new SqliteCheckCommand(staticCommand, _sqliteCommand.Clone());
		...
		internal protected override void ReadOwnerCommandParameter(string[] strArray)
		{
			foreach (string parameterName in strArray)
			{
				...
				{
					SqliteParameter param = dataCommand.Parameters[paramName] as SqliteParameter;
					SqliteParameter newParameter = (param as ICloneable).Clone() as SqliteParameter;
					_sqliteCommand.Parameters.Add(newParameter);
				}
			}
		}
