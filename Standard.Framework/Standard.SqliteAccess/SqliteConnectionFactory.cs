...
namespace Basic.SqliteAccess
{
	...
	internal sealed class SqliteConnectionFactory : ConnectionFactory
	{
		...
		{
			SqliteConnectionStringBuilder display = new SqliteConnectionStringBuilder();
		...
		public override DbConnection CreateConnection(ConnectionInfo info) { return new SqliteConnection(info.ConnectionString); }
		...
		/// <returns>DbConnection 的新实例。</returns>
		public override DbConnection CreateConnection(string connectionString) { return new SqliteConnection(connectionString); }

		/// <summary>
		/// 创建动态命令
		/// </summary>
		/// <returns>命令配置文件缓存</returns>
		public override DynamicCommand CreateDynamicCommand() { return new SqliteDynamicCommand(); }

		/// <summary>
		/// 创建静态命令
		/// </summary>
		/// <returns>命令配置文件缓存</returns>
		public override StaticCommand CreateStaticCommand() { return new SqliteStaticCommand(); }
		...
		{
			return new SqliteBulkCopyCommand(connection as SqliteConnection, tableInfo);
		}
