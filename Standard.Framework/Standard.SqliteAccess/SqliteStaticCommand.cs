...
namespace Basic.SqliteAccess
{
	...
	internal sealed class SqliteStaticCommand : StaticCommand, IDisposable, ICloneable
	{
		private readonly SqliteCommand sqlCommand;
		/// <summary>
		/// 初始化 SqliteStaticCommand 类的新实例。 
		/// </summary>
		public SqliteStaticCommand() : base(new SqliteCommand()) { sqlCommand = dataDbCommand as SqliteCommand; }

		/// <summary>
		/// 根据数据库命令，初始化 SqliteStaticCommand 类的新实例，主要克隆实例时使用。
		/// </summary>
		/// <param name="dbCommand">表示 SqlCommand 类实例。</param>
		private SqliteStaticCommand(SqliteCommand dbCommand) : base(dbCommand) { sqlCommand = dbCommand; }

		/// <summary>当前命令的数据库类型</summary>
		public override ConnectionType ConnectionType { get { return ConnectionType.SqlConnection; } }

		private SqliteCheckCommandCollection _CheckCommands = null;
		/// <summary>
		/// 检测命令集合
		/// </summary>
		internal protected override CheckCommandCollection CheckCommands
		{
			get
			{
				if (_CheckCommands == null)
					_CheckCommands = new SqliteCheckCommandCollection(this);
				return _CheckCommands;
			}
		}

		private SqliteNewValueCommandCollection _NewValues = null;
		/// <summary>
		/// 新值命令集合
		/// </summary>
		internal protected override NewValueCommandCollection NewValues
		{
			...
			{
				if (_NewValues == null)
					_NewValues = new SqliteNewValueCommandCollection(this);
				return _NewValues;
			}
		}
		...
		/// 重置数据库连接
		/// </summary>
		internal SqliteConnection Connection { get { return sqlCommand.Connection; } }

#if NET6_0_OR_GREATER
		/// <summary>创建批处理命令</summary>
		/// <returns>返回 SqlServerBatchCommand 的实例</returns>
		protected internal override BatchCommand CreateBatchCommand() { return new SqliteBatchCommand(this); }
#endif
		/// <summary>
		/// 重置数据库连接
		/// </summary>
		/// <param name="connection">一个 DbConnection，它表示到关系数据库实例的连接。 </param>
		internal void ResetConnection(SqliteConnection connection)
		{
		...
		internal protected override void ResetParameterValue(DbParameter dbParam, object value)
		{
			...
			else if (parameter.SqliteType == SqliteType.Text && value is int[])
			{
		...
		{
			return new SqliteCheckCommand(this);
		}
		...
		{
			return new SqliteNewValueCommand(this);
		}
		...
		/// </summary>
		/// <returns>作为此实例副本的新对象。</returns>
		public SqliteStaticCommand Clone()
		{
			lock (this)
			{
				SqliteCommand command = sqlCommand.Clone();
				SqliteStaticCommand staticCommand = new SqliteStaticCommand(command);
				CopyTo(staticCommand);
				if (_CheckCommands != null && _CheckCommands.Count > 0)
				{
					foreach (SqliteCheckCommand checkCommand in _CheckCommands)
					{
				...
				{
					foreach (SqliteNewValueCommand newValueCommand in _NewValues)
					{
