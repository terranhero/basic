...
namespace Basic.SqliteAccess
{
	...
	/// </summary>
	public sealed class SqliteBatchCommand : BatchCommand
	{
		/// <summary>
		/// 初始化 SqliteBatchCommand 类的新实例。 
		/// </summary>
		/// <param name="cmd">表示 <see cref="StaticCommand"/> 实例。</param>
		public SqliteBatchCommand(StaticCommand cmd) : base(cmd) { }
		...
		internal protected override async STT.Task<int> ExecuteAsync<TModel>(IEnumerable<TModel> entities, Action<DbParameter, TModel> paramSettings, int timeout)
		{
			...
			SqliteCommand command = staticCommand.DbCommand as SqliteCommand;
			//using (SqlBatch batch = new SqlBatch(command.Connection))
			//{
			//	foreach (TModel entity in entities)
			//	{
			//		SqliteCommand batchCommand = new SqliteCommand(staticCommand.CommandText);
			//		foreach (SqliteParameter parameter in command.Parameters)
			//		{
			//			SqliteParameter param = ((ICloneable)parameter).Clone() as SqliteParameter;
			...
			//		//batchCommand.Parameters.Add(new SqliteParameter(parameterName, i));
		...
		internal protected override async STT.Task<int> ExecuteAsync<TModel>(IEnumerable<TModel> entities, int timeout)
		{
			...
			//if (entities == null || entities.Any() == false) { return await Task.FromResult(0); }
			//SqliteCommand command = staticCommand.DbCommand as SqliteCommand;
			//using (SqliteBatch batch = new SqliteBatch(command.Connection))
			//{
			//	foreach (TModel entity in entities)
			//	{
		...
		//			foreach (TModel entity in entities)
		//			{
		//				SqliteCommand batchCommand = command.Clone(); ;
