...
namespace Basic.SqliteAccess
{
	...
	internal sealed class SqliteBulkCopyCommand : BulkCopyCommand, IDisposable
	{
		//private readonly SqliteCommand _SqlBulkCopy;
		...
		public SqliteBulkCopyCommand(SqliteConnection connection, TableConfiguration configInfo)
			: base(new SqliteCommand(), configInfo)
		{
			//_SqlBulkCopy = new SqliteCommand("", connection);
		...
		public override ConnectionType ConnectionType { get { return ConnectionType.SqliteConnection; } }
		...
		/// <param name="timeout">超时之前操作完成所允许的秒数。</param>
		/// <returns>受影响的行数。</returns>
		protected internal override void Execute<TR>(BaseTableType<TR> table, int timeout)
		{
			//return Task.CompletedTask;
		}
