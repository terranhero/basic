...
namespace Basic.DataAccess
{
	...
	/// 数据管理类
	/// </summary>
	public abstract class AbstractTableContext : AbstractDataContext
	{
		/// <summary>初始化 AbstractTableContext 类实例，此方法紧供子类继承调用</summary>
		protected AbstractTableContext() : base() { }
		...
		///// </summary>
		///// <returns>返回AbstractAccess子类的实例</returns>
		//protected abstract AbstractAccess CreateAccess();
		...
		/// <returns>如果执行成功返回0，执行失败则返回错误代码。</returns>
		public async Task<Result> CreateAsync(AbstractEntity[] entities)
		{
			using (AbstractAccess ac = CreateAccess(Connection, true))
			{
