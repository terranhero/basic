...
{
	/// <summary>可更新实体的动态命令</summary>
	/// <typeparam name="T">模型实体</typeparam>
	public interface IUpdateEntities<T> : IDisposable
...
, System.IAsyncDisposable
		 ...
		 where T : class
	{
		...
		/// <summary>通过传入实体模型，执行当前更新命令</summary>
		/// <param name="entities">需要执行命令的实体模型</param>
		/// <returns>执行Transact-SQL语句的返回结果</returns>
		System.Threading.Tasks.Task<Result> ExecuteAsync(T[] entities);

		/// <summary>通过传入实体模型，执行当前更新命令</summary>
		/// <param name="entity">需要执行命令的实体模型</param>
		/// <returns>执行Transact-SQL语句的返回结果</returns>
		System.Threading.Tasks.Task<Result> ExecuteAsync(T entity);

		/// <summary>通过传入匿名对象，执行当前更新命令</summary>
		...
		IUpdateEntities<T> Set<TP>(Expression<Func<T, TP>> predicate);

		/// <summary>使用Lambda表示式，设置更新命令的条件</summary>
		/// <param name="expression">用于测试每个元素是否满足条件的函数。</param>
		IUpdateEntities<T> Where<TP>(Expression<Func<T, TP>> expression);

		/// <summary>使用Lambda表示式，设置更新命令的条件</summary>
		/// <param name="predicate">用于测试每个元素是否满足条件的函数。</param>
		IUpdateEntities<T> Where(Expression<Func<T, bool>> predicate);
