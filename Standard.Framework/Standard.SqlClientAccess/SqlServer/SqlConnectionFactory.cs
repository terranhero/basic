...
namespace Basic.SqlServer
{
	...
	internal sealed class SqlConnectionFactory : ConnectionFactory
	{
		...
		//				sqlConStringBuilder.DataSource = item.Value;
		//			else if (item.Name == "UserID" || item.Name == "User ID")
		//				sqlConStringBuilder.UserID = item.Value;
		//			else if (item.Name == "Password")
		//				sqlConStringBuilder.Password = ConfigurationAlgorithm.Decryption(item.Value);
		//			else
		//				sqlConStringBuilder[item.Name] = item.Value;
		//		}

		//	}
		//	return sqlConStringBuilder.ConnectionString;
		//}
		...
		public override DynamicCommand CreateDynamicCommand() { return new SqlDynamicCommand(); }

		/// <summary>
		/// 创建静态命令
		/// </summary>
		/// <returns>命令配置文件缓存</returns>
		public override StaticCommand CreateStaticCommand() { return new SqlStaticCommand(); }
		...
		{
			return new SqlBatchCommand(connection as SqlConnection, tableInfo);
		}
