...
namespace Basic.PostgreSql
{
	...
	internal sealed class NpgNewValueCommandCollection : NewValueCommandCollection
	{
		private readonly NpgStaticCommand dbStaticCommand;
		/// <summary>
		/// 初始化 NpgNewValueCommandCollection 类的新实例。
		/// </summary>
		/// <param name="staticCommand">包含此检测命令集合的静态命令</param>
		public NpgNewValueCommandCollection(NpgStaticCommand staticCommand) : base(staticCommand) { dbStaticCommand = staticCommand; }

		/// <summary>
		/// 初始化 NpgNewValueCommandCollection 类的新实例，该类包含从指定列表中复制的元素。
		/// </summary>
		/// <param name="staticCommand">包含此检测命令集合的静态命令</param>
		/// <param name="list">从中复制元素的列表。</param>
		/// <exception cref="System.ArgumentNullException">list 参数不能为 null。</exception>
		public NpgNewValueCommandCollection(NpgStaticCommand staticCommand, IList<NewValueCommand> list)
		...
		{
			return new NpgNewValueCommand(dbStaticCommand);
		}
	}
}
