...
namespace Basic.PostgreSql
{
	...
	internal sealed class NpgCheckCommandCollection : Basic.DataAccess.CheckCommandCollection
	{
		private readonly NpgStaticCommand ownerCommand;
		/// <summary>
		/// 初始化 NpgCheckCommandCollection 类的新实例。
		/// </summary>
		/// <param name="staticCommand">包含此检测命令集合的静态命令</param>
		public NpgCheckCommandCollection(NpgStaticCommand staticCommand) : base(staticCommand) { ownerCommand = staticCommand; }

		/// <summary>
		/// 初始化 NpgCheckCommandCollection 类的新实例，该类包含从指定集合中复制的元素。
		/// </summary>
		/// <param name="staticCommand">包含此检测命令集合的静态命令</param>
		/// <param name="collection">从中复制元素的集合。</param>
		/// <exception cref="System.ArgumentNullException">collection 参数不能为 null。</exception>
		public NpgCheckCommandCollection(NpgStaticCommand staticCommand, IEnumerable<CheckCommand> collection)
		...
		public NpgCheckCommandCollection(NpgStaticCommand staticCommand, List<CheckCommand> list)
		...
		{
			return new NpgCheckCommand(ownerCommand);
		}
	}
}
