...
namespace Basic.PostgreSql
{
	...
	internal sealed class NpgStaticCommand : StaticCommand, IDisposable, ICloneable
	{
		private readonly NpgsqlCommand sqlCommand;
		/// <summary>
		/// 初始化 NpgStaticCommand 类的新实例。 
		/// </summary>
		public NpgStaticCommand() : base(new NpgsqlCommand()) { sqlCommand = dataDbCommand as NpgsqlCommand; }

		/// <summary>
		/// 根据数据库命令，初始化 SqlDynamicCommand 类的新实例，主要克隆实例时使用。
		/// </summary>
		/// <param name="dbCommand">表示 SqlCommand 类实例。</param>
		private NpgStaticCommand(NpgsqlCommand dbCommand) : base(dbCommand) { sqlCommand = dbCommand; }
		...
		internal protected override CheckCommandCollection CheckCommands
		{
			get
			{
				if (_CheckCommands != null) { return _CheckCommands; }
				_CheckCommands = new NpgCheckCommandCollection(this);
				return _CheckCommands;
			}
		}

		private NpgNewValueCommandCollection _NewValues = null;
		/// <summary>
		/// 新值命令集合
		/// </summary>
		internal protected override NewValueCommandCollection NewValues
		{
			...
			{
				if (_NewValues == null)
					_NewValues = new NpgNewValueCommandCollection(this);
				return _NewValues;
			}
		}
		...
		/// 重置数据库连接
		/// </summary>
		internal NpgsqlConnection Connection { get { return sqlCommand.Connection; } }

		/// <summary>
		/// 重置数据库连接
		/// </summary>
		/// <param name="connection">一个 DbConnection，它表示到关系数据库实例的连接。 </param>
		internal void ResetConnection(NpgsqlConnection connection)
		{
		...
		/// <param name="precision">获取或设置列中数据的最大大小（以字节为单位）。</param>
		/// <param name="scale">获取或设置数据库参数值解析为的小数位数。</param>
		internal void ConvertParameter(DbParameter parameter, DbTypeEnum dbType, byte precision, byte scale)
		{
			NpgsqlParameter sqlParameter = parameter as NpgsqlParameter;
			NpgParameterConverter.ConvertSqlParameterType(sqlParameter, dbType, precision, scale);
		}
		...
		{
			NpgsqlParameter parameter = sqlCommand.CreateParameter();
		...
		{
			return new NpgCheckCommand(this);
		}
		...
		{
			return new NpgNewValueCommand(this);
		}
		...
		/// </summary>
		/// <returns>作为此实例副本的新对象。</returns>
		public NpgStaticCommand Clone()
		{
			lock (this)
			{
				NpgsqlCommand command = sqlCommand.Clone();
				NpgStaticCommand staticCommand = new NpgStaticCommand(command);
				...
				{
					foreach (NpgCheckCommand checkCommand in _CheckCommands)
					{
				...
				{
					foreach (NpgNewValueCommand newValueCommand in _NewValues)
					{
