...
namespace Basic.EasyLibrary
{
	...
	/// 表示 EasyGrid 属性定义
	/// </summary>
	public class TreeTableOptions : DataGridOptions, IHtmlAttrs
	{
		/// <summary>
		///  初始化 EasyGridOptions 类的新实例，该实例为空且具有默认的初始容量，并使用键类型的默认相等比较器。
		/// </summary>
		public TreeTableOptions() : base("easyui-treetable") { }

		/// <summary>
		///  使用样式名称初始化 EasyGridOptions 类实例。
		/// </summary>
		/// <param name="classname">表示当前EasyGrid的样式名称。</param>
		public TreeTableOptions(string classname) : base() { className = classname; }
		...
		public TreeTableOptions(EasyOptions dictionary) : base(dictionary) { }
		...
		/// <exception cref="System.ArgumentNullException">dictionary 为 null。</exception>
		/// <exception cref="System.ArgumentException">dictionary 包含一个或多个重复键。</exception>
		public TreeTableOptions(IDictionary<string, object> dictionary) : base(dictionary) { className = "easyui-treetable"; }
