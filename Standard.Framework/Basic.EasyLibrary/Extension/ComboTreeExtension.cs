...
namespace Basic.EasyLibrary
{
	...
	/// 下拉树型控件扩展方法
	/// </summary>
	public static class ComboTreeExtension
	{
		/// <summary>
		...
		public static MvcHtmlString WebComboTreeFor<TM, TP>(this HtmlHelper<TM> html,
			Expression<Func<TM, TP>> expression)
		{
			return html.WebComboTreeFor<TM, TP>(expression, null);
		}
		...
		/// <returns>一个 HTML input 元素，其 type 特性针对表达式表示的对象中的每个属性均设置为“text”。</returns>
		public static MvcHtmlString WebComboTreeFor<TM, TP>(this HtmlHelper<TM> html, Expression<Func<TM, TP>> expression, IHtmlAttrs htmlAttrs)
		{
			if (htmlAttrs == null)
				htmlAttrs = new HtmlAttrs();
			htmlAttrs.className = "easyui-combotree";
