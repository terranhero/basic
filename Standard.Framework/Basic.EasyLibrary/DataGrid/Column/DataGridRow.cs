...
namespace Basic.EasyLibrary
{
	...
	public sealed class DataGridRow<T> : IEnumerable<DataGridColumn<T>> where T : class
	{
		...
		public DataGridRow(DataGrid<T> dg, int rowIndex) { _DataGrid = dg; _RowIndex = rowIndex; Height = SWUW.Unit.Empty; }
		...
		internal SWUW.Unit Height { get; private set; }

		/// <summary>用厘米为高度的数字初始化 Height 的新值。</summary>
		/// <param name="height">要设置的属性新值</param>
		/// <returns>返回当前对象。</returns>
		public DataGridRow<T> SetHeight(float height) { Height = new SWUW.Unit(height, SWUW.UnitType.Cm); return this; }

		/// <summary>用指定的长度初始化 System.Web.UI.WebControls.Unit 结构的新实例。</summary>
		/// <param name="height">要设置的属性新值</param>
		/// <returns>返回当前对象。</returns>
		public DataGridRow<T> SetHeight(string height) { Height = new SWUW.Unit(height); return this; }

		/// <summary>用像素为高度的数字初始化 Height 的新值</summary>
		/// <param name="height">要设置的属性新值</param>
		/// <returns>返回当前对象。</returns>
		public DataGridRow<T> SetHeight(int height) { Height = new SWUW.Unit(height); return this; }
