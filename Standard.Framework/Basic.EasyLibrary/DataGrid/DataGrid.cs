...
namespace Basic.EasyLibrary
{
	...
	public class DataGrid<T> : IDataGrid<T> where T : class
	{
		...
		public DataGrid(IBasicContext html, DataGridOptions options)
		{
			...
			mOptions = options == null ? new DataGridOptions() : options;
			EasyLibrarySection easySection = ConfigurationContext.GetEasyLibrary();
			EasyGridElement gridElement = easySection != null ? easySection.EasyGrid : null;

			if (gridElement != null)
			{
				if (gridElement.PageList != null)
					mOptions.PageList = gridElement.PageList.Split(',').Select(m => { return Convert.ToInt32(m); }).ToArray();
				mOptions.CheckOnSelect = gridElement.CheckOnSelect;
				mOptions.SelectOnCheck = gridElement.SelectOnCheck;
				mOptions.Fit = gridElement.Fit; mOptions.PageSize = gridElement.PageSize;
			}
		}
		...
		/// </summary>
		public void Dispose() { mBasic.Writer.Flush(); }
		...
		{
			string httpMethod = mBasic.Request.HttpMethod.ToUpper();
			if (httpMethod == "GET")
				return RenderMode.Normal;
			else if (httpMethod == "POST")
			{
				string renderModeString = mBasic.Request.QueryString["ExportMode"];
				if (string.IsNullOrWhiteSpace(renderModeString))
					renderModeString = mBasic.Request.Form["ExportMode"];
				if (renderModeString != null)
				{
					...
					else if (Enum.TryParse<RenderMode>(mBasic.Request.Form["ExportMode"], false, out mode))
						return mode;
				}
				return RenderMode.Json;
			}
		...
		private IRender<T> GridRender(RenderMode mode)
		{
			...
			else if (mode == RenderMode.TableContent) { return new ContentRender<T>(this); }
		...
		public void RenderReport(IPagination<T> dataSource)
		{
			...
			if (render == RenderMode.ExcelXlsx)
			{
				...
				{
					FileName = DownloadFileAttribute.GetFileName(mBasic.Request)
				};
				rd.Render(mBasic, dataSource);
			}
			else { new ReportRender<T>(this).Render(mBasic, dataSource); }
		}
		...
		public void Render(IPagination<T> dataSource)
		{
			...
			rd.FileName = DownloadFileAttribute.GetFileName(mBasic.Request);
			rd.Render(mBasic, dataSource);
		}
		...
		{
			new ContentRender<T>(this).Render(mBasic, dataSource);
		}
		...
		{
			new XlsxRender<T>(this).Render(mBasic, dataSource);
		}
		/// <summary>
		...
		{
			new DataRender<T>(this).Render(mBasic, dataSource);
		}
